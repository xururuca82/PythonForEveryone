...
def make_question():
    ...
    op = random.randint(1,3)    # 1~3 사이의 임의수를 op에 저장합니다.

    # 문자열 변수 q에 문제를 만듭니다.
    # 첫 번째 숫자를 q에 저장합니다.
    q = str(a)                  # a 값(정수)를 문자열로 바꾸어 저장합니다.
    ...
    # 두 번째 숫자를 q에 저장합니다.
    q = q + str(b)              # b 값(중수)을 문자열로 바꾸어 q에 추가합니다.
    
    # 만들어진 문제를 돌려줍니다.
    return q

# 정담/오답 횟수를 저장할 변수 sc1과 sc2를 0으로 초기화합니다.
sc1=0
sc2=0
...
for x in range(5):              # 다섯 문제를 풀어 봅니다.
    ...
    # 컴퓨터가 계산한 결과 eval(q)의 값과 사용자가 입력한 결과(r)를 비교합니다.
    if eval(q) == r:
        print("정답!")
        sc1 = sc1 + 1
    else:
...
print("정답 :", sc1, "오답 :", sc2)
if sc2 == 0:                    # 오답이 0개인 때(전부 정답을 맞혔을 때)
    print("당신은 천재입니다.")
