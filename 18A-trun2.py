...
def play():             # 게임을 실제로 플레이하는 함수
    ...
    if random.randint(1,5) == 3:    # 1~5 사이에 뽑은 수가 3이면(20%확률)
    ...
    if speed > 15:
        speed = 15
    te.forward(speed)
    if t.distance(te) < 12:     # 주인공와 악당의 거리가 12보다 작으면
                                # 게임을 종료합니다.
    ...
    if playing:    # 주인공와 악당의 거리가 12이상이면(멀면)
...
t.listen()                      # 거북이 그래픽 창이 키보드 입력 받도록 합니다.
